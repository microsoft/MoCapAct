...
def make_env(seed=0, training=True):
    ...
    env = env_util.make_vec_env(
        ...
        wrapper_kwargs=dict(embed_dim=embed_dim, embed_max=FLAGS.embed_max, embed_std=1.1, include_embed_in_obs=True)
    )
    env = VecMonitor(env)
    env = VecNormalize(
        env,
        ...
        norm_obs_keys=observables.CMU_HUMANOID_OBSERVABLES,
    )
...
def main(_):
    ...
    seq_steps = FLAGS.model.config.bc_seq_steps if hasattr(FLAGS.model.config, 'bc_seq_steps') else 1
    ...
    model = PPOBC(policy, env, n_steps=int(FLAGS.n_steps/FLAGS.n_workers),
                ...
                seed=FLAGS.seed, verbose=1, policy_kwargs=policy_kwargs,
                device=FLAGS.device, bc_coef=FLAGS.bc_coef, bc_dataloader=dataloader,
                bc_freq=FLAGS.bc_freq)
