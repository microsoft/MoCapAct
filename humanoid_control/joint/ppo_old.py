...
class PPOBC(PPO):
    ...
    def __init__(
        ...
        self.embed_kl_coef = embed_kl_coef
    ...
    def _make_prior_embedding_distribution(self, observations):
        prev_embed = observations['embedding']
        # Unnormalize observed embedding if we use `VecNormalize`
        if isinstance(self.env, VecNormalize) and 'embedding' in self.env.norm_obs_keys:
            prev_embed_np = self.env._unnormalize_obs(
                prev_embed.cpu().numpy(),
                self.env.obs_rms['embedding']
            )
            prev_embed = torch.tensor(prev_embed_np, device=prev_embed.device)
        
        return Independent(Normal(self.policy.embedding_correlation*prev_embed, self.policy.embedding_std_dev), 1)
    ...
    def train(self) -> None:
        ...
        embed_kls, delta_embed_means, delta_embed_stds, embed_means, embed_stds = [], [], [], [], []
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                values, log_prob, embed_distribution = self.policy.evaluate_actions(rollout_data.observations, actions)

                # Make prior Gaussian to be used for embedding KL regularization
                prior_embed_distribution = self._make_prior_embedding_distribution(rollout_data.observations)
                kl_div = kl_divergence(embed_distribution, prior_embed_distribution).mean()
                ...
                embeds = actions[..., -self.policy.embed_size:]
                embed_kls.append(kl_div.item())
                delta_embed_means.append((embed_distribution.mean - prior_embed_distribution.mean).abs().mean().item())
                delta_embed_stds.append(embed_distribution.stddev.mean().item())
                embed_means.append(embeds.mean().item())
                embed_stds.append(embeds.std().item())
                ...
                loss = policy_loss + self.embed_kl_coef * kl_div + self.vf_coef * value_loss + self.bc_freq*self.bc_coef * bc_loss
        ...
        if hasattr(self.policy, "log_embed_std"):
            self.logger.record("train/embed_std", torch.exp(self.policy.log_embed_std).mean().item())
        ...
        self.logger.record("embed/embedding_kl", np.mean(embed_kls))
        self.logger.record("embed/delta_embedding_mean", np.mean(delta_embed_means))
        self.logger.record("embed/delta_embedding_std", np.mean(delta_embed_stds))
        self.logger.record("embed/embedding_mean", np.mean(embed_means))
        self.logger.record("embed/embedding_std", np.mean(embed_stds))
