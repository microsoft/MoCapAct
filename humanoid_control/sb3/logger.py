...
class CSVOutputFormat(KVWriter):
    ...
    def __init__(self, filename: str, flush_every: Optional[int] = None):
    ...
    def write(self, key_values: Dict[str, Any], key_excluded: Dict[str, Union[str, Tuple[str, ...]]], step: int = 0) -> None:
        ...
        # Flush the output to the file
        if self.flush_every:
            if time.time() - self.start_time >= 60*self.flush_every:
                self.file.flush()
                self.start_time = time.time()
        else:
            self.file.flush()
...
class TensorBoardOutputFormat(KVWriter):
    ...
    def __init__(self, folder: str, flush_every: Optional[int] = None):
...
def make_output_format(_format: str, log_dir: str, log_suffix: str = "", flush_every: Optional[int] = None) -> KVWriter:
    """
    ...
    if _format == "stdout":
        ...
        return CSVOutputFormat(os.path.join(log_dir, f"progress{log_suffix}.csv"), flush_every=flush_every)
    elif _format == "tensorboard":
        return TensorBoardOutputFormat(log_dir, flush_every=flush_every)
    else:
...
def configure(folder: Optional[str] = None, format_strings: Optional[List[str]] = None, flush_every: Optional[int] = None) -> Logger:
    """
    ...
    :param flush_every: how often (in minutes) to flush the logs
    ...
    output_formats = [make_output_format(f, folder, log_suffix, flush_every) for f in format_strings]
