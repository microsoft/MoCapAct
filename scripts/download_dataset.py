    ...
    if not os.path.exists(local_dest_path):
        ...
        for file in files:
            if not blob_connector.blob_exists(file):
                raise f"File {file} does not exist in the dataset, please check the available files with 'python download_dataset.p -l' flag."
...
if __name__ == '__main__':
    ...
    if args.list:
        ...
        download_dataset_from_url(dataset_url=args.url, local_dest_path='./data')
